...
public abstract class BaseConstraintDEAlgorithm implements ConstrainedDEAlgorithm{
	...
	protected abstract List<Long> doFind(Long[] factors, Long result, Long[][] ranges, List<List<Long>> histories);
	
	@Override
	public List<Long> findSolution(Long[] factors, Long result, Long[][] ranges, List<Long> baseResult,List<List<Long>> histories) {
		init(factors, ranges,baseResult,result);
		List<Long> solutions =  doFind(getNewFactors(),getNewResult(),ranges,histories);
		context.set(null);
		return solutions;
	}
	...
	public List<List<Long>> findSolutions(Long[] factors, Long result,
		...
		init(factors, ranges,baseResult,result);
		while(count > 0){
			List<Long> solution =  doFind(getNewFactors(),getNewResult(),ranges,new ArrayList<List<Long>>());
			if(solution == null)
				count = 0;
			else{
	...
	protected void init(Long[] factors,Long[][] ranges,List<Long> baseSolution,Long result) {
		if(context.get() == null){
			context.set(new HashMap<String,Object>());
		}
		List<GreatestCommonDivisor> allGcd = GreatestCommonDivisor.getSequenceGCD(factors);
		...
		List<GreatestCommonDivisor> newGcd = GreatestCommonDivisor.getSequenceGCD(newFactors);
	...
	protected <T> T getContextObject(String key) {
		return context.get().containsKey(key) ? (T) context.get().get(key) : null;
	}
	...
	protected List<Long> filt(int index, int excludeIndex, Long[] factors, Long[][] ranges, Long result,
			List<Long> solutions, List<Long> newSolutions) {
		...
		if(excludeIndex != -1) {
			if(index == factors.length - 1) {
				...
				gcdValue = GreatestCommonDivisor.getGCD(new Long[] {gcdValue,factors[excludeIndex]}).getGcd();
			}
		...
		/*
		 * for(int i = 0;i < solutions.size();i++) { remainResult = - solutions.get(i) *
		 * factors[i]; }
		 */
		...
		Long lastSolution = Long.MIN_VALUE;
		if (isOrdered() && index > 0) {
			lastSolution = (index == excludeIndex + 1 ? (excludeIndex > 0 ? solutions.get(index - 2) : Long.MIN_VALUE) : solutions.get(index - 1));
		}
		...
		for (Long testSolution : newSolutions) {
			...
			if (tmpResult <= maxResult && tmpResult >= minResult
					...
					&& (!isOrdered || testSolution > lastSolution)
					) {
	...
	protected boolean isOrdered() {
		Boolean isOrdered = getContextObject(ContextKeys.IS_ORDERED);
		if(isOrdered == null) {
			isOrdered = false;
		}
		return isOrdered;
	...
	protected List<Long> pickUpFinalResult(List<List<Long>> candidateSolutions,Long[] factors,
			Long[][] ranges,Long result){
	...
	protected List<Long> findFinalSolutions(Stack<SearchStep> searchPath,Long result,
			Long[] factors,Long[][] ranges,int round){

		List<Long> testSolutions = toSolutions(searchPath);
		Long tmpResult = recalculateResult(result, testSolutions, factors);
		List<List<Long>> possibleSolution = 
				MathUtils.findDEPossibleSolution(new Long[] {factors[round],factors[round + 1]}, tmpResult, 
	...
	private Stack<SearchStep> changePath(Stack<SearchStep> searchPath) {
		...
		do {
			...
			else if(lastFinalStep.getLastSelector() < lastFinalStep.getSolutions().size() - 1) {
				lastFinalStep.setLastSelector(lastFinalStep.getLastSelector() + 1);
				break;
			}
			else {
	...
	protected List<Long> pickUpFinalResult(Long[] factors,
			Long[][] ranges,Long result){
		...
		while(round < maxRound && round >= 0) {
			...
			SearchStep lastStep = searchPath.isEmpty() ? null : searchPath.peek();
			List<Long> testSolutions = toSolutions(searchPath);
			if(isFinalRound(round)) {
				List<Long> finalSolution = findFinalSolutions(searchPath, result, factors, ranges, round);
			...
			if(round == getSpecialIndex()) {
				if(!(lastStep != null && lastStep.getRound().equals(getSpecialIndex()))) {
					searchPath.push(new SearchStep(round, 0, getSpecialIndexNewSolutions(ranges[round])));
					round ++;
				}
				else {
					searchPath.pop();
				...
				if(lastStep != null && lastStep.getRound() == round) {
					...
					Long tempResult = recalculateResult(result, testSolutions, factors);
					List<Long> newSolutions = getCandiadateSolution(round,tempResult);
					newSolutions  = filt(round, getSpecialIndex(), factors, ranges, tempResult,
							testSolutions, newSolutions);
					...
					if(newSolutions.size() == 0) {
						...
						searchPath.push(new SearchStep(round, selectIndex, newSolutions));
						round ++;
					}
				}
			}
	...
	protected class SearchStep{
		...
		public SearchStep(Integer round, Integer lastSelector, List<Long> solutions) {
			super();
			this.round = round;
			this.lastSelector = lastSelector;
			this.solutions = solutions;
		...
		public SearchStep clone(){
			...
			SearchStep newStep = new SearchStep(new Integer(round), new Integer(lastSelector), newSolution);
			return newStep;
		}
	...
	protected Long recalculateResult(Long result,List<Long> solutions,Long[] factors) {
	...
	protected void initCongruenceSolution(Long[] factors,Long[][] ranges,Long modular,Long result) {
		List<SolutionInformation> solutionInfos = Lists.newArrayList();
		for(int i = 0;i < factors.length;i++) {
			solutionInfos.add(new SolutionInformation(i, factors, modular, ranges[i]));
		}
	...
	protected class SolutionInformation{
		...
		public SolutionInformation(Integer index,Long[] factors,Long modular,Long[] range) {
		...
		private void init(Long modular,Long[] range) {
			...
			while(beginSolution <= range[1]) {
				remainder = MathUtils.getLeastRemainder(beginSolution * factor, modular);
	...
	protected Stack<SearchStep> getLastFinishedSteps(){
		if(context.get().containsKey(ContextKeys.LAST_FINISHED_STEPS)){
			return (Stack<SearchStep>) context.get().get(ContextKeys.LAST_FINISHED_STEPS);
		}
		return null;
	}
