...
public class SimpleMailSender implements EMailService {
	...
	public SimpleMailSender(final String smtpHostName, final int sendPort, final String username,
			final String password) {
		init(username, password, smtpHostName, sendPort);
	}
	...
	private void init(String username, String password, String smtpHostName, int sendPort) {
		// 初始化props
		props.put("mail.smtp.auth", "true");
		props.put("mail.smtp.host", smtpHostName);
		...
		authenticator = new MailAuthenticator(username, password);
		// 创建session
		//Authenticator.requestPasswordAuthentication(new Inetadre, sendPort, "smtp", username, password);
		session = Session.getInstance(props, authenticator);
	}
	...
	public void send(String recipient, String subject, Object content) throws AddressException, MessagingException {
		...
		send(message,new InternetAddress[]{new InternetAddress(recipient)});
	}
	
	private void send(Message message,Address[] address) throws MessagingException{
